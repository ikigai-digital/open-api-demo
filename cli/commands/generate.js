...
const TMP_DIR = 'tmp'
const GENERATOR_DIR = 'node_modules/openapi/generators/ikigai-nodejs-express-server'
const REQUIRED_CONTRACT_FIELDS = ['type', 'boundedContext', 'name', 'version']
const CONTRACT_TYPES = ['provider', 'client']

const generateSingleFile = async ({ filePath, type, buildDir, name, version }) => {
  try {
    const { stdout: globalNpmLocation } = await execAsync('npm list -g | head -1')
    const providerGeneratorDir = `${join(
      globalNpmLocation.replace('\n', ''),
      GENERATOR_DIR,
    )}/target/ikigai-nodejs-express-server-openapi-generator-1.0.0.jar`
    const generatorName = type === 'provider' ? 'ikigai-nodejs-express-server' : 'typescript-axios'
    ...
    console.log({
      ...
      globalNpmLocation,
      name,
      providerGeneratorDir,
      generatorName,
    })

    const projectName = `@ikigai-digital/${name.toLowerCase()}-${type}`
    const versionInitial = `V${version.split('.')[0]}`

    const additionalArgs =
      type === 'provider'
        ? `--custom-generator="${providerGeneratorDir}" -p npmName=${projectName},npmDescription=\\"Autogenerated ${versionInitial} server stub for ${name}\\"`
        : `--p npmName=${projectName},supportsES6=false,withInterfaces=true --git-repo-id open-api-demo --git-user-id ikigai-digital`

    const { stdout, stderr } = await execAsync(
      `npx @openapitools/openapi-generator-cli generate ${additionalArgs} -g ${generatorName} -i ${filePath} -o ${buildDir}`,
    )
