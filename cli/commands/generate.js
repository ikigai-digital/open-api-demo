...
const GENERATOR_DIR = 'node_modules/openapi/generator/ikigai-nodejs-express-server'
...
const generateSingleFile = async ({ filePath, type, buildDir, name, version }) => {
  try {
    ...
    const providerGeneratorDir = join(globalNpmLocation.replace('\n', ''), GENERATOR_DIR)
    const generatorType =
      type === 'provider'
        ? `${providerGeneratorDir}/target/ikigai-nodejs-express-server-openapi-generator-1.0.0.jar`
        : 'typescript-axios'
    ...
    const additionalArgs =
      type === 'provider'
        ? `-p npmName=${projectName},npmDescription=\\"Autogenerated ${versionInitial} server stub for ${name}\\"`
